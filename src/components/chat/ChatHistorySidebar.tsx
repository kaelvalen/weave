        ...
        <div className="flex items-center gap-2 overflow-hidden flex-1">
          ...
          {isEditing ? (
            <div className="flex items-center gap-1 w-full" onClick={(e) => e.stopPropagation()}>
              ...
              />
              <Check
                className="w-3.5 h-3.5 text-green-500 cursor-pointer"
                onClick={() => handleRenameSubmit(session.id)}
              />
              <X
                className="w-3.5 h-3.5 text-destructive cursor-pointer"
        ...
        {!isEditing && (
          ...
          >
            <Button
              variant="ghost"
              size="icon"
              className="h-6 w-6 hover:bg-muted-foreground/20 text-muted-foreground"
    ...
    <div className="w-72 h-full border-r bg-muted/30 flex flex-col flex-shrink-0 rounded-l-2xl overflow-hidden">
      ...
      <ScrollArea className="flex-1">
        <div className="p-2 flex flex-col gap-4">
          {sessions.length === 0 ? (
            <div className="text-xs text-muted-foreground text-center py-8">No previous chats</div>
          ) : (
            <>
