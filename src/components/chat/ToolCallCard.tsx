        ...
        <div className="mt-2 space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[11px] font-mono">
            <div className="bg-red-500/10 border border-red-500/20 rounded p-2 overflow-x-auto whitespace-pre">
              <div className="text-red-500/70 mb-1 text-[9px] uppercase tracking-wider font-sans font-bold">
                Removed
              </div>
            ...
            <div className="bg-green-500/10 border border-green-500/20 rounded p-2 overflow-x-auto whitespace-pre">
              <div className="text-green-500/70 mb-1 text-[9px] uppercase tracking-wider font-sans font-bold">
                Added
              </div>
              <div className="text-green-500">{newStr}</div>
            </div>
          </div>
          {(res.backed_up as boolean) && (
            <div className="text-[10px] text-muted-foreground mt-1 text-right">
              Backed up to .weave.bak
            </div>
          )}
        </div>
      );
    ...
    if (call.capability === 'coder.write_file') {
      return (
        <div className="mt-2 flex items-center gap-2 p-2 bg-muted/30 border rounded text-xs">
          ...
          <span>
            Wrote <strong>{res.bytes_written as number}</strong> bytes to{' '}
            <code className="bg-background px-1 py-0.5 rounded border">{res.path as string}</code>
          </span>
        </div>
      );
    }
      ...
      return (
        <div className="mt-2 space-y-2">
          <div className="flex items-center gap-1.5 text-xs font-medium">
            ...
            {call.capability === 'coder.run_tests' ? 'Test Results' : 'Check Results'} (
            {res.check_type as string})
          </div>
          <div className="text-[11px] font-mono bg-background/50 border rounded p-2 overflow-x-auto whitespace-pre max-h-[250px] overflow-y-auto">
            ...
            <div className={success ? 'text-foreground' : 'text-red-400'}>
              {(res.stdout as string) || (res.stderr as string) || 'No output'}
            </div>
          </div>
          {res.tests_passed !== null && res.tests_passed !== undefined && (
            <div className="flex gap-4 text-[11px]">
              <span className="text-green-500 font-medium">
                {res.tests_passed as number} Passed
              </span>
              {(res.tests_failed as number) > 0 && (
                <span className="text-red-500 font-medium">
                  {res.tests_failed as number} Failed
                </span>
              )}
            </div>
          )}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-muted-foreground/90 text-xs">
            {isCoderPlugin ? (
              <PlayCircle className="w-3.5 h-3.5 inline mr-1.5 text-blue-500 opacity-80" />
            ) : (
